...
describe("Dates", () => {
  describe("dayRange", () => {
    ...
    it("should disregard time", () => {
      const dates = Dates.dayRange(
        new Date(2019, 5, 4, 10, 11, 30),
        new Date(2019, 5, 1, 12, 10, 1)
      );
      expect(dates).to.eql([
        Dates.endOfDay(new Date(2019, 5, 1)),
        Dates.endOfDay(new Date(2019, 5, 2)),
  ...
  it("should strip milliseconds from date", () => {
    ...
    expect(Dates.stripMillis(date)).to.eql(
      new Date(2019, 6, 18, 10, 10, 10, 0)
    );
  });
});
